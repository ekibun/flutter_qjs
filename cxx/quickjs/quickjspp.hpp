...
struct function
{
    ...
    alignas(std::max_align_t) char functor[1];
...
public:
    ...
    public:
        Module(JSContext * ctx, const char * name) : ctx(ctx)
        {
        ...
        Module& add(const char *_name, JSValue value)
        {
            exports.push_back({_name, {ctx, value}});
            JS_AddModuleExport(ctx, m, _name);
            return *this;
        }

        Module& add(const char * _name, Value value)
        {
            assert(value.ctx == ctx);
            exports.push_back({_name, std::move(value)});
            JS_AddModuleExport(ctx, m, _name);
            return *this;
        }
